...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    /**
     * Metodo que gestiona el intento de autenticación
     */
    @Override
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            // Mapeo del JSON a la entidad UserEntity
            userEntity = new ObjectMapper().readValue(request.getInputStream(), UserEntity.class);
            username = userEntity.getUsername();
            password = userEntity.getPassword();

        } catch (Exception e) {
            log.error("Error de intento de autenticación ".concat(e.getMessage()));
        }
        ...
        // Retornamos la Authentication
        return getAuthenticationManager().authenticate(authenticationToken);
    }

    /**
     * Si la autenticación es existosa
     */
    @Override
